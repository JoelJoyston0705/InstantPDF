...
import { Upload, Type, CheckCircle, AlertCircle, Download, File } from 'lucide-react';
...
export default function WatermarkPdfPage() {
    ...
    const handleSubmit = async () => {
        ...
        formData.append('text', watermarkText); // Send user's text

        try {
            const API_URL = import.meta.env.VITE_API_URL || 'https://instantpdf-production.up.railway.app';
            // Cache Busted URL
            const response = await fetch(`${API_URL}/edit/watermark-pdf?t=${Date.now()}&text=${encodeURIComponent(watermarkText)}`, {
                method: 'POST',
                body: formData,
            });
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.detail || 'Watermark addition failed');
            }
        ...
        <div className="min-h-screen pt-24 pb-16 px-6 page-transition">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="text-center mb-16 animate-fade-in-up">
                    <div className="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-gradient-to-br from-cyan-500 to-blue-500 shadow-2xl mb-8 text-white transform hover:scale-110 transition-transform">
                        <Type size={40} strokeWidth={2} />
                    </div>
                    <h1 className="mb-4" style={{ letterSpacing: '-0.022em' }}>Watermark PDF</h1>
                    <p className="subtitle max-w-xl mx-auto">
                        Add text watermarks to your PDF documents to protect your intellectual property.
                    </p>
                </div>
                ...
                <div className="tool-card shadow-premium animate-scale-in max-w-2xl mx-auto">
                    ...
                    {status === 'converting' && (
                        <div className="p-20 text-center animate-fade-in">
                            ...
                            <p className="text-gray-500">Adding watermark to your document.</p>
                        </div>
                    )}
                    ...
                    {status === 'done' && (
                        <div className="p-20 text-center animate-scale-in">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                <a
                                    href={downloadUrl}
                                    download={`${file.name.split('.')[0]}_watermarked.pdf`}
