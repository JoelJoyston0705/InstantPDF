...
import { Upload, RotateCw, RotateCcw, CheckCircle, AlertCircle, Download, File, RefreshCw } from 'lucide-react';
...
export default function RotatePdfPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Send rotation as query parameter
            const response = await fetch(`${API_URL}/edit/rotate-pdf?t=${Date.now()}&rotation=${rotation}`, {
                method: 'POST',
                body: formData,
            });
            ...
            if (!response.ok) {
                ...
                throw new Error(errData.detail || 'Rotation failed');
            }
        ...
        <div className="min-h-screen pt-24 pb-16 px-6 page-transition">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="text-center mb-16 animate-fade-in-up">
                    <div className="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-gradient-to-br from-lime-500 to-green-500 shadow-2xl mb-8 text-white transform hover:scale-110 transition-transform">
                        <RotateCw size={40} strokeWidth={2} />
                    </div>
                    <h1 className="mb-4" style={{ letterSpacing: '-0.022em' }}>Rotate PDF</h1>
                    <p className="subtitle max-w-xl mx-auto">
                        Permanently rotate your PDF pages to the correct orientation.
                    </p>
                </div>
                ...
                <div className="tool-card shadow-premium animate-scale-in max-w-2xl mx-auto">
                            ...
                            {!file ? (
                                ...
                                <div className="animate-fade-in">
                                    ...
                                    <div className="mb-8 max-w-sm mx-auto">
                                        <label className="block text-sm font-medium text-gray-700 mb-4">Rotation Direction</label>
                                        <div className="flex justify-center gap-4">
                                            <button
                                                onClick={() => setRotation(90)}
                                                className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${rotation === 90 ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300'}`}
                                            >
                                                <RotateCw size={24} />
                                                <span className="text-sm font-medium">90° CW</span>
                                            </button>

                                            <button
                                                onClick={() => setRotation(180)}
                                                className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${rotation === 180 ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300'}`}
                                            >
                                                <RefreshCw size={24} />
                                            ...
                                            <button
                                                ...
                                                className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${rotation === 270 ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 hover:border-gray-300'}`}
                                            >
                                                <RotateCcw size={24} />
                                                <span className="text-sm font-medium">90° CCW</span>
                                            </button>
                                        </div>
                                    </div>
                    ...
                    {status === 'converting' && (
                        <div className="p-20 text-center animate-fade-in">
                            ...
                            <p className="text-gray-500">Adjusting document orientation.</p>
                        </div>
                    )}
